...
<div class="row tab-content">
    ...
    <div role="tabpanel" class="row tab-pane fade in active" id="profil">
        {{ BootForm::openHorizontal(2, 10)->put()->action(URL::route('user.update', $user->id)) }}
            <div class="col-sm-8 col-sm-offset-2">
                <div class="col-sm-9 col-sm-offset-3"><h3>Coordonnées de {{ ucfirst($user->firstname) }}</h3></div>
                    {{ Form::token() }}
                    {{ BootForm::bind($user) }}
                    ...
                    @endif
                    {{ BootForm::textarea('Plus de détail <span class="textarea-subtext">Dites en un peu plus sur vous même et/ou vos activités...</span>', 'description')->placeHolder("Dites en un peu plus sur vous même et/ou vos activités...")->id('editor1') }}
                        <script>
